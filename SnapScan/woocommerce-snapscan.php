    ...
    class WC_Ecentric_Snapscan extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
            ...
            if (isset($mp_settings)) {
                ...
                $settings_webhook_auth = $settings['webhook_auth_key'];
                $settings_api_key = $settings['api_key'];
                // check if values are present, so that empty settings doesn't delete
                if (isset($settings_webhook_auth)) {
                    $this->update_option('webhook_auth', $settings_webhook_auth);
                } else {
                    if ($this->settings['logging'] == 'yes') {
                        SnapLogger::log('[App] No webhook auth key available in parsed settings: ' . $mp_settings);
                    }
                }
                if (isset($settings_api_key)) {
                    $this->update_option('snap_api', $settings_api_key);
                    $this->update_option('snap', $settings_api_key);
                } else {
                    if ($this->settings['logging'] == 'yes') {
                        SnapLogger::log('[App] No api key available in parsed settings: ' . $mp_settings);
                    }
                }
